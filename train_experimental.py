...
from looper_experimental import Looper
...
# temp command: python trainByBatchLinux.py 10 "-d egg-fullsize-pt-presample-compare-2021-03-23 -n FCRN_A -lr 0.0000000125 -e 150 -hf 0.5 -vf 0.5 --val_interval 2 -rot --plot --batch_size 4 --rand_samp_mult 20 --config /media/Synology3/Robert/objects_counting_dmap/configs/presampleComparison_dynamic_sample_mae_loss_2021-04-05.json"
...
def train(
    ...
    lr_history = {}
    lr_history_filename = (
        f"{dataset_name}_{network_architecture}_lr_history"
    ...
    if lr_scheduler == "step":
        lr_scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=5, gamma=0.95
        )
    ...
    for i, epoch in enumerate((range(epochs))):
        ...
        if i % val_interval == 0:
            ...
            if newBest:
                for f in glob(
                    f"{dataset_name}_{network_architecture}_{train_ts}_best_"
                    f"*_{platform.node()}.pth".replace(":", "-")
                ):
                    os.unlink(f)
                torch.save(
                    network.state_dict(),
                    f"{dataset_name}_{network_architecture}_{train_ts}_best_epoch{i+1}"
                    f"_{platform.node()}.pth".replace(":", "-"),
                )
        ...
        pr.disable()
        pr.dump_stats('fcrn_a_performance_stats')
        print("\n", "-" * 80, "\n", sep="")
