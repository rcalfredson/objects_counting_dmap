...
def get_egg_image_paths(held_out=False):
    all_images = glob('egg_source%s/*_dots*'%('/heldout' if held_out else ''))
    shuffle(all_images)
    return all_images
...
def generate_egg_heldout_data():
    ...
    in_channels = 3
    _, valid_h5 = create_hdf5('egg-eval',
                              train_size=0,
                              valid_size=None,
                              img_size=(None, None),
...
def generate_egg_data(fullSize=False):
    """Generate HDF5 files for egg-laying images."""
    training_image_names = glob('egg_source/%strain/*_dots*'%('fullsize_' if fullSize else ''))
    validation_image_names = glob('egg_source/%svalid/*_dots*'%('fullsize_' if fullSize else ''))
    train_h5, valid_h5 = create_hdf5('egg%s'%('-fullsize' if fullSize else ''),
    ...
    fill_h5(train_h5, training_image_names, oneDSetPerImg=fullSize)
    fill_h5(valid_h5, validation_image_names, oneDSetPerImg=fullSize)
